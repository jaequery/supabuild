...
`/supabuild` is one mode-routed entry point with five flows: a
Team-Lead-orchestrated multi-agent build (§A), parallel
design-variant exploration (§B), autonomous Linear backlog
burndown (§C), a thin git-worktree wrapper (§D), and the GitHub
Projects v2 mirror of §C (§E). The first whitespace-delimited token
of `$ARGUMENTS` selects the flow; sections cross-reference each
other via the `§A`/`§B`/`§C`/`§D`/`§E` letter prefix.
...
- `worktree` → §D (worktree flow). Remainder of args = task
  description.
- `github` → §E (github-projects-v2 flow). Remainder of args = optional
  task description (auto-creates an issue) / assignee filter / flags.
...
The five sections below are self-contained — every internal cross-
reference within a section uses the section letter as a prefix
(`§A.3`, `§B.6`, `§C.3d.5`, `§D.5`, `§E.3`) so there's no ambiguity
between, e.g., "§3" in build (a build round) and "§3" in linear (a
per-ticket loop).
...
the repo's Build Queue project, run the §A build flow against that
...
- `--project <num>` — explicit project number. Default: auto-detect
  the `Build Queue` project owned by the repo's owner (auto-created
  on first run).
...
2. **Find or create the Build Queue project.** (Override with
   `--project <num>` to skip detection.)
   ```bash
   PROJECT_NUM=$(gh project list --owner "$OWNER" --format json \
     | jq -r '.projects[] | select(.title=="Build Queue") | .number' \
     | head -1)
   ```
   If `PROJECT_NUM` is empty, auto-create:
   ```bash
   PROJECT_JSON=$(gh project create --owner "$OWNER" \
     --title "Build Queue" --format json)
   PROJECT_NUM=$(echo "$PROJECT_JSON" | jq -r '.number')
   ```
   Print the project URL so the user can bookmark it.
...
6. **Print resolved setup** so the user audits before processing:
   ...
   Repo:    $REPO_FULL
   Project: #$PROJECT_NUM (Build Queue)
